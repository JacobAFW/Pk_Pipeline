...
As a consensus VCF has not be built, we must build our own. Here we use a combination of previously studies samples of known high quality, and our own high quality samples to build a consensus VCF.
This follows a similar workflow to the standard calling of variants, where we mark duplicates, and then skip realignment and recalibration, going straight to haplotype calling. 
...
bcftools mpileup --threads 20 -f $INDEXTDIR -b input_bam_files.list \
   | bcftools call -m -Oz -o $OUTDIR/PK_samtools.raw.vcf.gz
...
Versions: 1.13 (bcftools)
File: samtools_merge_vcfs.pbs

Arguments:
 - forece-samples - the same samples were called on both variant callers and so the names in the two vcf files are the same. This argument will assign a prefix of '2:' to the names from the second VCF file, so that we can differentiate between the two.
...
The resulting TSV is then wrangled in R (script below), producing several summary statistices, along with a TSV containing a list of the variants that were called by both variant calling tools.
This TSV then has the header removed and is converted into a txt file containing a list of all the consensus variants.
We then use fgrep to search the original merged VCF for all patterns (consensus variants) in the previously created txt file.
This is then combined with the first 85 lines of the merged VCF to create a new consensus VCF file.
...
Version(s) = 1.12 (samtools), 1.8.0_131-b1 (Java), 2.21.9 (picard) & 3.2.2 (GATK).
...
The VCF file for the known variant sites (VCF) had to be amended to accomodate GATK's nuances:

'sed /ID=VDB/d PK_consensus.vcf | sed /Number=R/d > PK_consensus_corrected.vcf'

Arguments:
 
To calculate the average number of homozygous genotypes per site (needed for -hets and -indelHeterozygosity arguments)
...
`awk -F\0\/0: '!/^ *#/ {total += NF-1; count++} END { print total/count }' PK_consensus_corrected_indels.vcf`
`awk -F\0\/0: '!/^ *#/ {total += NF-1; count++} END { print total/count }' PK_consensus_corrected_snps.vcf`
...
KNOWNSITES="/home/jwestaway/pk_pipeline/ZB_100/outputs/vcf_consensus/PK_consensus.vcf"
