...
bcftools query -i 'FMT/GQ>30 & FMT/DP>10' -f '%CHROM %POS %ID %REF %ALT [\t%SAMPLE DP=%DP GQ=%GQ MQ=%MQ PL=%PL]\n' merged_GATK_samtools_variants.vcf.gz > merged.tsv
...
bcftools view merged_GATK_samtools_variants.vcf.gz | grep -f grep_patterns.txt > filtered.vcf

# Concatenate the vcf header in the variants called by both callers
cat vcf_head.vcf filtered.vcf > PK_consensus.vcf

# Remove intermediate files
rm -f vcf_head.vcf grep_patterns.txt filtered.vcf
